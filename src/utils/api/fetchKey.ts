...
const ENDPOINT = `${API_URL}/fetchkey`;

export type fetchkeyAPIReq = {
  path: string;
};

export type fetchkeyAPIRes = {
  res: any;
  // error: boolean;
};

export const fetchkeyAPI = async (path: string) => {
    const req: fetchkeyAPIReq = {
        path: path
      };
  ...
  console.log("fetchkey Response body", responseBody);
  ...
  try {
    ...
    return responseBody as fetchkeyAPIRes;
  } catch (error) {
